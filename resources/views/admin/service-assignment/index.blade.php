                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-4 text-end">
                            <button type="button" class="btn btn-light me-2" data-bs-toggle="modal" data-bs-target="#bulkAssignModal">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                         ...
                                         <tr>
                                             <td>
                                                 <input type="checkbox" name="selected_services[]" value="{{ $request->id }}" class="form-check-input service-checkbox">
                                             </td>
                                             <td>
                                ...
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            ...
                                            <small class="text-muted">{{ $assignment->units->name ?? 'N/A' }} - {{ $assignment->types->title ?? 'N/A' }}</small>
                                            <br>
                                            <small class="text-success">
                                                <i class="ti ti-user"></i> {{ $assignment->maintainers && $assignment->maintainers->id ? $assignment->maintainers->name : 'Unassigned' }}
                                            </small>
                                        </div>
                                        <div class="text-end">
                                            <span class="badge bg-success">{{ __('Assigned') }}</span>
                                            <br>
                                            <small class="text-muted">{{ \Carbon\Carbon::parse($assignment->request_date)->format('H:i') }}</small>
                                        </div>
                                    </div>
                                    <div class="mt-2">
                                        <button type="button" class="btn btn-outline-warning btn-sm" 
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-transparent border-0">
                    <h6 class="mb-0 text-info">
                        <i class="ti ti-calendar"></i> {{ __('Tomorrow\'s Assignments') }} ({{ $today->copy()->addDay()->format('M d, Y') }})
                    </h6>
                </div>
                <div class="card-body">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="assigned_date" class="form-control" value="{{ $today->format('Y-m-d') }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('Notes') }}</label>
                            <textarea name="notes" class="form-control" rows="2" placeholder="{{ __('Optional notes for the operator...') }}"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('admin.service-assignment.reassign') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-info">
                        <i class="ti ti-info-circle"></i> {{ __('Currently assigned to:') }} <strong id="currentMaintainer"></strong>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('New Assignment Date') }}</label>
                            <input type="date" name="new_date" class="form-control" value="{{ $today->format('Y-m-d') }}" required>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.service-assignment.bulk-assign') }}">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="ti ti-info-circle"></i> {{ __('Select services from the table above, then assign them to an operator.') }}
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label">{{ __('Notes') }}</label>
                        <textarea name="notes" class="form-control" rows="3" placeholder="{{ __('Notes for all selected services...') }}"></textarea>
                    </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="submit" class="btn btn-primary" id="bulkAssignBtn" disabled>{{ __('Assign Selected Services') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            select.html('<option value="">{{ __("Select Operator") }}</option>');
                         ...
                         if (response.maintainers.length > 0) {
                 response.maintainers.forEach(function(maintainer) {
                     select.append('<option value="' + maintainer.id + '">' + maintainer.name + ' (' + maintainer.type + ')</option>');
                 });
             } else {
        ...
        error: function() {
            $('#assignMaintainerSelect').html('<option value="">{{ __("Error loading operators") }}</option>');
        }
    });
}
...
function updateSelectedServices() {
    ...
    if (selectedIds.length > 0) {
        ...
        $('#selectedServicesDisplay').html('<span class="badge bg-primary me-1">' + selectedIds.length + ' services selected</span>');
